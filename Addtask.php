   ...
   // define variables and set to empty values
	{
		...
		$query2 = "SELECT sales_repid from tblcontact where id='$company'";
		$result2 = $db->query($query2);
		$row2 = mysqli_fetch_array($result2,MYSQLI_ASSOC);
		$salesrep = $row2['sales_repid'];
		
		//get product detail
		$query = "SELECT product_name, price from tblproduct where prdID='$productID'";
		...
		$query="INSERT INTO tbltask (quantity, product, subprice, total, contact_id, sales_rep_id, notes, status,date_open)
			VALUES ('$quantity', '$prdName', '$prdPrice', '$total', '$company', '$salesrep', '$notes', 'PENDING',CURDATE())";
           $result = $db->query($query);
			if(! $result)
			 {
		...
		<body>	
			...
			<div class="contact-page-area section-gap">
				...
				<div class="centerItem">
					<h3 class="text-heading">New Task</h3>
                </div>	
						<div class="rowForm">				
						<div class="col-lg-8">
							<form class="form-area " action="" method="post" class="contact-form text-right">
								<div class="row">
									...
									<div class="col-lg-6 form-group">
										<input name="fldquantity" placeholder="Enter the Quantity" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter First Name'" class="common-input mb-20 form-control" required="" type="number">
										
										
										<label>Product</label>
										 <?php
										
										$query = "SELECT prdID,product_name from tblproduct";
										$result = $db->query($query);
										$rowcount=mysqli_num_rows($result);
										$all_property = array();										 
										 ...
										 if ($rowcount>0)
										{
											     echo '<select name="fldproductID" class="common-input mb-20 form-control">';
												 echo '<option value="" selected disabled hidden>Choose here</option>';
												 while ($row = mysqli_fetch_array($result,MYSQLI_ASSOC)){

													 echo '<option value="'.$row['prdID'].'">'.$row['product_name'].'</option>';
										...
										$salesid= $_SESSION['id'];
										$query = "SELECT ID, company from tblcontact";
										$result = $db->query($query);
										$rowcount=mysqli_num_rows($result);
										$all_property = array();										 
										 ...
										 if ($rowcount>0)
										{
											     echo '<select name="fldcompany" class="common-input mb-20 form-control">';
												 echo '<option value="" selected disabled hidden>Choose here</option>';
												 while ($row = mysqli_fetch_array($result,MYSQLI_ASSOC)){
									...
									<div class="col-lg-6 form-group">
										<textarea class="common-textarea form-control" name="fldnotes" placeholder="notes" onfocus="this.placeholder = ''" onblur="this.placeholder = 'notes'" required=""></textarea>
